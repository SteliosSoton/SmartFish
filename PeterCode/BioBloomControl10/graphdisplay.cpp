...
GraphDisplay::GraphDisplay(QString graphType, int ideal, QString potMac, QWidget *parent): QWidget(parent)
{//initialise members with ideal vvalue
    ...
    QNetworkAccessManager *manager = new QNetworkAccessManager(this);
       ...
       QString postKey = "mac";
       QString postValue = potMac; //GET JOE TO PASS THE MAC ADDRESS ID FOR THE RIGHT PLANT TO HERE
                               // CAN GET HIM TO DO IT TOMORROW
...
void GraphDisplay:: graphDataFetchFinished(QNetworkReply* reply)
{
   ...
   int i=0;
    ...
    while(i<points_list.count())
    {
        ...
        qint64 timeOfValue=QDateTime().currentDateTime().toMSecsSinceEpoch()-(points_list.count()*6-i)*60000;//check my arithmetic, this should find the date/time the datapoint was recorded
        chart->addPoint(timeOfValue, points_list[i].toInt());
    i+=6;
    }
